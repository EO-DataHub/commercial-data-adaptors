...
def _read_credentials(workspace: str) -> Credentials:
    ...
    try:
        ...
        r = v1.read_namespaced_secret(f"oauth-{_PROVIDER}", namespace)  # pyright: ignore

        return Credentials(**r.data)  # pyright: ignore
    except ApiException as e:
        if e.status == 404:
            raise Exception(f"No Open Cosmos credentials found for workspace '{workspace}'. "
                "Please link your Open Cosmos account in your workspace settings.",
            ) from e
        if e.status == 403:
            raise Exception(f"Permission denied reading Open Cosmos credentials for workspace '{workspace}'.",
            ) from e
        raise Exception(f"Failed to read Open Cosmos credentials: {e.reason}") from e
    except Exception as e:
        raise Exception(f"Unexpected error reading Open Cosmos credentials: {e}") from e



def _get_credentials(workspace: str) -> Credentials:
    ...
    """
    credentials = _read_credentials(workspace)

    if credentials.expires_at - _EXPIRY_MARGIN > datetime.now():
        return credentials

    logging.info("Open Cosmos access token has expired; refreshing.")
    return _refresh_credentials(workspace, credentials)



def _refresh_credentials(workspace: str, credentials: Credentials) -> Credentials:
    ...
    try:
        ...
        raise Exception("Failed to refresh Open Cosmos access token.") from e
    j = r.json()
    ...
    try:
        ...
        raise Exception(f"Failed to persist refreshed Open Cosmos credentials: {e.reason}") from e

    # Reread the updated credentials from Kubernetes to ensure they are up-to-date.
    return _read_credentials(workspace)
...
def get_contract_info(workspace: str) -> ContractInfo:
    ...
    credentials = _get_credentials(workspace)
    ...
    if len(policies) == 0:
        raise IndexError("No policies found for organization")
...
def get_access_token(workspace: str) -> str:
    credentials = _get_credentials(workspace)
