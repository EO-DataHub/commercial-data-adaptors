...
product_bundle_map = {
    "PSScene": {
        "Visual": {"name": "visual"},
        "General Use": {"name": "analytic_8b_udm2,analytic_udm2"},
...
def update_stac_item_success(
    ...
    write_stac_item_and_catalog(stac_item, file_name, collection_id, order_name, workspace, workspaces_bucket)
...
def update_stac_item_failure(
    ...
    write_stac_item_and_catalog(stac_item, file_name, collection_id, order_name, workspace, workspace_bucket)
...
def update_stac_item_ordered(
    ...
    try:
        ingest_stac_item(stac_item, s3_bucket, pulsar_url, workspace, collection_id, item_id)
...
def ingest_stac_item(
    ...
    item_key = f"{workspace}/{parent_catalog_name}/planet/{collection_id}/{item_id}.json"
    ...
    producer = pulsar_client.create_producer(topic="transformed", producer_name=f"data_adaptor-{workspace}-{item_id}")
    output_data = {
...
async def get_existing_order_details(workspace: str, order_name: str) -> dict:
...
def get_credentials() -> dict:
    ...
    return {
        "AccessKeyId": get_aws_api_key_from_secret("planet-aws-access-key-id", "planet-aws-access-key-id"),
        "SecretAccessKey": get_aws_api_key_from_secret("planet-aws-secret-access-key", "planet-aws-secret-access-key"),
    }
...
def main(
    ...
    for stac_item in stac_items:
        ...
        if collection_id not in product_bundle_map:
        ...
        try:
            ...
            download_and_store_locally(commercial_data_bucket, f"{delivery_folder}/{order_id}", order_id)
        except Exception as e:
